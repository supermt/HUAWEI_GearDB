...
static inline __be32 to_be32(uint32_t x) { return (FORCE __be32)htonl(x); }

// static inline __be16 to_be16(uint16_t x) { return (FORCE __be16)htons(x); }
...
static inline uint32_t from_be32(__be32 x) { return ntohl((FORCE uint32_t)x); }

// static inline uint16_t from_be16(__be16 x) { return ntohs((FORCE uint16_t)x);
// }
...
static bool cmp_sha1(const uint8_t *a, const uint8_t *b) {
...
int btree_open(struct btree *btree, const char *fname) {
  ...
  if (btree->fd < 0) return -1;

  struct btree_super super;
  if (read(btree->fd, &super, sizeof super) != (ssize_t)sizeof super) return -1;
...
void btree_close(struct btree *btree) {
  ...
  for (i = 0; i < CACHE_SLOTS; ++i) {
    if (btree->cache[i].offset) free(btree->cache[i].table);
  }
}
    ...
    *(uint32_t *)sha1 = -1;
    ...
    if (offset) {
      ...
      *(__be64 *)sha1 = to_be64(offset);
      ...
      while (free_len > len) {
        free_len >>= 1;
        free_chunk(btree, offset + free_len, free_len);
      }
    } else {
      in_allocator = 0;
    }
...
static off_t lookup(struct btree *btree, off_t table_offset,
                    const uint8_t *sha1);
off_t insert_toplevel(struct btree *btree, off_t *table_offset, uint8_t *sha1,
...
static off_t remove_table(struct btree *btree, struct btree_table *table,
  ...
  if (sha1) memcpy(sha1, table->items[i].sha1, SHA1_LENGTH);
  ...
  if (left_child != 0 && right_child != 0) {
    ...
    off_t new_offset;
    if (rand() & 1) {
      new_offset = take_largest(btree, left_child, table->items[i].sha1);
      table->items[i].child = to_be64(collapse(btree, left_child));
    } else {
      new_offset = take_smallest(btree, right_child, table->items[i].sha1);
...
void *btree_get(struct btree *btree, const uint8_t *sha1, size_t *len) {
  ...
  lseek(btree->fd, offset, SEEK_SET);
  struct blob_info info;
  if (read(btree->fd, &info, sizeof info) != (ssize_t)sizeof info) return NULL;
...
int btree_delete(struct btree *btree, const uint8_t *c_sha1) {
  ...
  lseek(btree->fd, offset, SEEK_SET);
  struct blob_info info;
  if (read(btree->fd, &info, sizeof info) != sizeof info) return 0;
