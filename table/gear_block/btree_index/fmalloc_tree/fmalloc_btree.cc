...
#define BT_MAX_ENT                                                           \
  ((long)((PAGE_SIZE - sizeof(struct bt_latch) - sizeof(struct bt_pg_hdr)) / \
          128) -                                                             \
   1)
...
#define BT_LOCK(_pg)                                   \
  do {                                                 \
    uint8_t expected = 0;                              \
    while (!(_pg)->latch.lock.compare_exchange_strong( \
...
#define BT_TRYLOCK(_pg)                                \
  ({                                                   \
    bool ret;                                          \
    uint8_t expected = 0;                              \
    if (!(_pg)->latch.lock.compare_exchange_strong(    \
            expected, 1, std::memory_order_release)) { \
      ret = false;                                     \
    } else {                                           \
      ret = true;                                      \
...
static uint32_t jenkins_one_at_a_time_hash(const char *key, size_t length) {
  size_t i = 0;
  uint32_t hash = 0;
  while (i != length) {
    hash += key[i++];
    ...
    for (const std::string &s : fields) {
      ...
      while (field) {
        if (s.size() == field->kl &&
          ...
          results.push_back(std::pair<std::string, std::string>(
              s, std::string(&field->data[field->kl + 1])));
          found = true;
          break;
        }
        field = field->ptr;
      ...
      if (!found)
        results.push_back(
            std::pair<std::string, std::string>(s, std::string("")));
    }
  } else {
    struct bt_item *field = item->ptr;
    while (field) {
      results.push_back(std::pair<std::string, std::string>(
          std::string(field->data), std::string(&field->data[field->kl + 1])));
      field = field->ptr;
    }
  }
}

static inline void __bt_put_result(struct bt_cur *cur, struct bt_item *old_item,
                                   struct bt_item *new_item) {
  ...
  while (new_field) {
    bool replaced = false;
    struct bt_item *new_tmp = new_field->ptr, *old_field = old_item->ptr,
                   *old_prev = old_item;
    new_field->ptr = NULL;
    while (old_field) {
    ...
    if (!replaced) old_prev->ptr = new_field;
    new_field = new_tmp;
  }
...
static int bt_fetch_cb_delete(struct bt_cur *cur, struct bt_pg *_p,
                              uint16_t _i);
...
static inline void __set_time(struct bt_cur *cur, bool enter) {
  ...
  clock_gettime(CLOCK_REALTIME, &ts);
  if (enter)
    cur->enter_time.store(ts.tv_sec * 1000000000UL + ts.tv_nsec,
                          std::memory_order_release);
  else
    cur->exit_time.store(ts.tv_sec * 1000000000UL + ts.tv_nsec,
                         std::memory_order_release);
}

static inline void set_enter_time(struct bt_cur *cur) { __set_time(cur, true); }

static inline void set_exit_time(struct bt_cur *cur) { __set_time(cur, false); }

static int __fetch_item(struct bt_cur *cur, struct bt_pg *pg, bool backward,
                        bool locking, bool collid, bool *mustcheck_backward) {
  int err = 0, i, inc;
  uint16_t start, end;
  struct bt_pg *next;
  struct bt_item *item;
  ...
  if (next && locking &&
      (next == cur->path[cur->depth - 1]._c_p_pg ||
       next == cur->path[cur->depth - 1]._c_n_pg)) {
  ...
  if (next) {
    ...
    err = __fetch_item(cur, next, backward, locking, true, NULL);
    if (locking) BT_RELEASE(next);
    if (err == -EAGAIN) goto out0;
  }
out1:
  if (!locking) {
    if (err == -EAGAIN) goto out0;
...
static int fetch_item(struct bt_cur *cur, struct bt_pg *pg, bool locking) {
   ...
   * locking mode traverses leaf's next hops, and previous hops while taking the
   * locks of them. so, we can trust the values of next and previous hop
   * pointers at ever node.
   *
   * in non-locking mode, we do not have a lock, and if we detect changes while
   * returning from the neighbor hops, we throw away the result and return to
   * retry.
   */

  /*
   * because we accept a same key id with different key strings to
   * cope with hash collision, potentially, a split produces the following
   * situation.
   *
   *
   * < before >
   ...
   *
   *
   * in the above case, traversal for a key 11 will reache the entry whose
   * keystr is "y". however, if we look for "z", we need to go the next entry.
   * if we want "x", we need to go to the previous leaf.
   *
   * in this implementation, we start from checking the current leaf and,
   * if the current_leaf->entry[0] has the key that we want to have, we go to
   * the previous leaf, because previous_leaf->entry[used_ent-1] may also have
   * the same key.
   *
   */
  ...
  if (err == 0 && go_backward) {
     ...
     */
    struct bt_pg *pre;
    if (!locking) ver = pg->latch.ver.load(std::memory_order_acquire);
    pre = (struct bt_pg *)pg->hdr.__pre64;
    if (!pre) goto out;
    if (pre && locking &&
        (pre == cur->path[cur->depth - 1]._c_p_pg ||
         pre == cur->path[cur->depth - 1]._c_n_pg)) {
...
static int bt_fetch_cb_lookup(struct bt_cur *cur, struct bt_pg *pg,
                              uint16_t idx) {
  int err = 1;
...
static int bt_add_ent(struct bt_pg *pg, uint64_t key, uint64_t val) {
  int i = 0;
  for (i = 0; i < pg->hdr.__used_ent16; i++) {
    if (key < pg->__key64[i]) break;
  }
  memmove(&pg->__key64[i + 1], &pg->__key64[i],
          sizeof(uint64_t) * (pg->hdr.__used_ent16 - i));
  memmove(&pg->__val64[i + 1], &pg->__val64[i],
...
static void bt_split(struct bt_cur *cur, struct bt_pg *ppg, struct bt_pg *cpg) {
  ...
  memcpy(&newpg->__key64[0], &cpg->__key64[BT_MAX_ENT - BT_MAX_ENT / 2],
         sizeof(uint64_t) * (BT_MAX_ENT - BT_MAX_ENT / 2));
  memcpy(&newpg->__val64[0], &cpg->__val64[BT_MAX_ENT - BT_MAX_ENT / 2],
         sizeof(uint64_t) * (BT_MAX_ENT - BT_MAX_ENT / 2 + 1));
  ...
  if (!cpg->hdr.leaf)
    i = bt_add_ent(ppg, cpg->__key64[BT_MAX_ENT - BT_MAX_ENT / 2 - 1],
                   (uint64_t)cpg);
  else {
    if (cpg->hdr.__nxt64) {
...
static int bt_fetch_cb_insert(struct bt_cur *cur, struct bt_pg *pg,
                              uint16_t idx) {
...
static int __bt_insert(struct bt_cur *cur, struct bt_pg *pg, uint16_t depth) {
  ...
  if (pg->hdr.leaf) {
    ...
    if (BT_IS_LOCKED(cur->path[depth - 1].pg) ||
        cur->path[depth - 1].ver != cur->path[depth - 1].pg->latch.ver.load(
       ...
       *
       *
       * in the upper case, let's say we are trying to add a key of 12 in the <
       * before > state. first, in the 1st step, we will reach the leaf at
       * 0xfff000 in < before >. in the 2nd phase, someone has splitted the
       * leaf, and the state goes to < after >. next, as the 3rd phase, we are
       * still looking at the leaf at 0xfff000, and will hold a lock.
       * afterwards, in the 4th step, we will add 12 to the leaf at 0xfff000.
       *
       * now, what we will see will be the following tree. 12 is inserted in the
       * wrong place.
       *
       *
       * node | 3 | 9 | 18 |
       ...
       *
       *
       * potentially, after we hold a lock of a leaf, the leaf may not be the
       * right place to add a key that we are trying to insert anymore.
       *
       * in order to detect that we are in the wrong place, we check the
       * parent's modification AFTER we get the lock of the leaf. because, any
       * node and leaf won't be splitted while its lock is taken by someone
       * else. we need to have a lock of this leaf first, for avoiding split on
       * this leaf.
       *
       * now, we check the parent, IF the parent is changed, here may not be the
       * right place. so, we give up this trial and go back to retry.
       *
       */
      err = -EAGAIN;
      BT_RELEASE(pg);
    ...
    if (BT_IS_LOCKED(pg) ||
        cur->path[depth].ver != pg->latch.ver.load(std::memory_order_acquire)) {
      /*
       * at this point, freshness of the pointer to a child node has to be
       * guaranteed in order to be able to detect change on this node from the
       * point of view of this child node.
       *
       * at least, up to here, we have chosen a correct path that is not
       * modified by anybody. consequently, change of the version number tells
       * us the modification on this node.
       */
      err = -EAGAIN;
      goto out;
    }
    cur->path[depth].in_lock = 0;
    /*
     * while we are going through here, someone may modify this node because we
     * don't have the lock, and the child node that we are trying to looking
     * into may not be the correct one anymore. but, we will perform detection
     * in the leaf, so just keep going. if we find we came to a wrong leaf, we
     * will retry.
     */
    ...
    if (err == -EAGAIN)
      ...
      if (cpg->hdr.__used_ent16 == BT_MAX_ENT) bt_split(cur, pg, cpg);
      BT_RELEASE(cpg);
      if (!cur->path[depth - 1].in_lock) BT_RELEASE(pg);
    }
  }
out:
  return err;
...
static int bt_insert(struct bt_cur *cur, struct bt_pg *pg) {
  ...
  fmalloc_set_target(cur->bt->fi);
recheck:
  if (cur->waitlock.load(std::memory_order_acquire)) goto recheck;
  bt_do_pg_free(cur);
  set_enter_time(cur);
...
retry:
  ...
  if (err == -EAGAIN)
    ...
    if (cpg->hdr.__used_ent16 == BT_MAX_ENT) bt_grow(cur, pg);
    BT_RELEASE(cpg);
    BT_RELEASE(pg);
  }
  set_exit_time(cur);
...
static int bt_fetch_cb_delete(struct bt_cur *cur, struct bt_pg *_p,
  ...
  if (del_pg && (del_pg->__key64[del_ent] == cur->key) && i == 0) {
    ...
    if (pg->hdr.__nxt64) next = (struct bt_pg *)pg->hdr.__nxt64;
    ...
    if (!cur->path[di->del_depth].in_lock) BT_RELEASE(del_pg);
  }
  ffree(item);
  if (_p != pg) {
...
static int bt_path_lock_for_delete(struct bt_cur *cur, uint16_t depth) {
  ...
  if (cur->path[depth].ver != pg->latch.ver.load(std::memory_order_acquire)) {
     ...
     * anymore as the same reason as insert. so, just give up and return to
     * retry.
     */
    err = -EAGAIN;
    cur->path[depth].in_lock = 0;
   ...
   * and we can spend plenty of time to wait until someone releases the child's
   * lock.
   */
  ...
  if (lpg->hdr.leaf && (lpg->hdr.__used_ent16 == 1) &&
      (pg->hdr.__used_ent16 == 1))
   ...
   * because, now we have a lock of the parent node, and the child to parent
   * locking yields and go back to retry if the executor detects our lock. So,
   * let's just wait until the child finds us and releases its lock.
   */
   ...
   * we remove the leaf. so we need to take another lock in the parent of this
   * node.
   */
...
static void bt_path_unlock_for_delete(struct bt_cur *cur, uint16_t depth) {
  ...
  for (i = 0; i <= depth; i++) {
    if (cur->path[i].in_lock) {
      ...
      if (cur->path[i]._c_n_pg) BT_RELEASE(cur->path[i]._c_n_pg);
      if (cur->path[i]._c_p_pg) BT_RELEASE(cur->path[i]._c_p_pg);
...
static int __bt_delete(struct bt_cur *cur, struct bt_pg *pg, uint16_t depth) {
  ...
  if (pg->hdr.leaf) {
    ...
    if (BT_IS_LOCKED(cur->path[depth - 1].pg)) goto leaf_again;
    if (cur->path[depth - 1].ver !=
        cur->path[depth - 1].pg->latch.ver.load(std::memory_order_acquire)) {
    /*
    ...
    if (pg->hdr.__used_ent16 == 1) {
      err = bt_path_lock_for_delete(cur, depth - 1);
      if (err == -EAGAIN) goto leaf_retry;
      /*
       * when this leaf disappears, we need to have other prev and next leaves'
       * locks. while we have the lock of this leaf lock, neighbors won't
       * disappear because they need the lock we have for removing themselves.
       */
  ...
  if (err == -EAGAIN)
    ...
    if (cpg->hdr.__used_ent16 != 0) goto unlock;
    ...
    if (_r->hdr.__used_ent16 > 1) {
      if (!cpg->hdr.leaf) {
        ...
        memmove(&(_r->__key64[0]), &(_r->__key64[1]),
                sizeof(uint64_t) * (_r->hdr.__used_ent16 - 1));
        memmove(&(_r->__val64[0]), &(_r->__val64[1]),
                sizeof(uint64_t) * _r->hdr.__used_ent16);
      } else {
      ...
      if (!cpg->hdr.leaf) {
        ...
        memmove(&(_p->__key64[_idx]), &(_p->__key64[_idx + 1]),
                sizeof(uint64_t) * (_p->hdr.__used_ent16 - _idx - 1));
        memmove(&(_p->__val64[_idx]), &(_p->__val64[_idx + 1]),
                sizeof(uint64_t) * (_p->hdr.__used_ent16 - _idx));
        if (_r->hdr.__used_ent16)
  ...
  unlock:
    if (cur->path[depth]._c_n_pg) BT_RELEASE(cur->path[depth]._c_n_pg);
    if (cur->path[depth]._c_p_pg) BT_RELEASE(cur->path[depth]._c_p_pg);
    if (!cpg_freed) BT_RELEASE(cpg);
    if (!cur->path[depth - 1].in_lock) BT_RELEASE(pg);
  }
out:
  return err;
...
retry:
  ...
  if (cur->path[1].in_lock) BT_RELEASE(cpg);
  if (cur->path[0].in_lock) BT_RELEASE(pg);
  if (err == -EAGAIN) goto retry;
  set_exit_time(cur);
  return err;
}
...
class BTKVSImpl : public BKVS {
 private:
  ...
  struct bt_cur *fetch_cur(const char *table) {
    ...
    if (search == trees.end()) {
      ...
      search = trees.find(std::string(table));
      if (search == trees.end()) {
        tbl = doCreate();
        assert(tbl);
      ...
      tbl = search->second;
    std::unordered_map<std::string, struct bt_cur *> &hash_cur =
        *__thread_bkvs_cur;
    ;
 ...
 public:
  ...
  ~BTKVSImpl() {
    if (__thread_bkvs_cur) delete __thread_bkvs_cur;
  }
  ...
  int Put(const char *table, const char *key, uint32_t kl,
    ...
    struct bt_cur *cur = fetch_cur(table);
    item = (struct bt_item *)bt_alloc_obj(cur->bt,
                                          sizeof(struct bt_item) + kl + 1);
    assert(item);
    item->kl = kl;
    ...
    for (std::pair<std::string, std::string> &p : values) {
      field = (struct bt_item *)bt_alloc_obj(
          cur->bt,
          sizeof(struct bt_item) + p.first.size() + p.second.size() + 2);
      assert(field);
      field->ptr = NULL;
  ...
  int Put(const char *table, const char *key, uint32_t kl, const char *val,
          uint32_t vl) override {
    (void)vl;
    std::vector<std::pair<std::string, std::string>> v;
    std::pair<std::string, std::string> p =
        std::pair<std::string, std::string>(std::string(""), std::string(val));
    v.push_back(p);
