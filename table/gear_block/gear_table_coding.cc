...
// Return 0 for error
size_t EncodeSize(GearTableEntryType type, uint32_t key_size,
                  char* out_buffer) {
  out_buffer[0] = type << 6;
...
inline Status GearTableKeyDecoder::DecodeSize(uint32_t start_offset,
                                              GearTableEntryType* entry_type,
                                              uint32_t* key_size,
                                              uint32_t* bytes_read) {
  ...
  *entry_type = static_cast<GearTableEntryType>(
...
IOStatus GearTableKeyEncoder::AppendKey(const Slice& key,
  ...
  if (parsed_key.sequence == 0 && parsed_key.type == kTypeValue) {
    ...
    *offset += key_to_write.size() - 8;
    meta_bytes_buf[*meta_bytes_buf_size] = GearTableFactory::kValueTypeSeqId0;
...
Status GearTableKeyDecoder::ReadInternalKey(
  ...
  if (tmp_slice[user_key_size] == GearTableFactory::kValueTypeSeqId0) {
...
Status GearTableKeyDecoder::NextGearEncodingKey(uint32_t start_offset,
                                                ParsedInternalKey* parsed_key,
                                                Slice* internal_key,
                                                uint32_t* bytes_read,
                                                bool* /*seekable*/) {
...
Status GearTableKeyDecoder::NextPrefixEncodingKey(uint32_t start_offset,
                                                  ParsedInternalKey* parsed_key,
                                                  Slice* internal_key,
                                                  uint32_t* bytes_read,
                                                  bool* seekable) {
...
Status GearTableKeyDecoder::NextKey(uint32_t start_offset,
                                    ParsedInternalKey* parsed_key,
...
Status GearTableKeyDecoder::NextKeyNoValue(uint32_t start_offset,
                                           ParsedInternalKey* parsed_key,
                                           Slice* internal_key,
                                           uint32_t* bytes_read,
                                           bool* seekable) {
