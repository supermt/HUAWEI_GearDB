...
Compaction* GearCompactionPicker::PickCompaction(
  ...
  GearCompactionBuilder builder(ioptions_, icmp_, cf_name,
                                     mutable_cf_options, vstorage, this,
                                     log_buffer);
  return builder.PickCompaction();
}

void GearCompactionBuilder::SortedRun::Dump(char* out_buf,
                                                 size_t out_buf_size,
...
void GearCompactionBuilder::SortedRun::DumpSizeInfo(
...
std::vector<GearCompactionBuilder::SortedRun>
GearCompactionBuilder::CalculateSortedRuns(
    const VersionStorageInfo& vstorage) {
  std::vector<GearCompactionBuilder::SortedRun> ret;
...
// Gear style of compaction. Pick files that are contiguous in
...
Compaction* GearCompactionBuilder::PickCompaction() {
  ...
  if (sorted_runs_.size() == 0 ||
    ...
    TEST_SYNC_POINT_CALLBACK(
        "GearCompactionBuilder::PickCompaction:Return", nullptr);
    return nullptr;
  }
  VersionStorageInfo::LevelSummaryStorage tmp;
  ROCKS_LOG_BUFFER_MAX_SZ(
      log_buffer_, 3072,
      "[%s] Gear: sorted runs: %" ROCKSDB_PRIszt " files: %s\n",
  ...
  if (c == nullptr &&
    ...
    if ((c = PickCompactionToReduceSizeAmp()) != nullptr) {
      ROCKS_LOG_BUFFER(log_buffer_, "[%s] Gear: compacting for size amp\n",
                       cf_name_.c_str());
    } else {
      ...
      if ((c = PickCompactionToReduceSortedRuns(ratio, UINT_MAX)) != nullptr) {
        ROCKS_LOG_BUFFER(log_buffer_,
                         "[%s] Gear: compacting for size ratio\n",
                         cf_name_.c_str());
      } else {
        ...
        if (num_sr_not_compacted >
          ...
          if ((c = PickCompactionToReduceSortedRuns(UINT_MAX, num_files)) !=
              nullptr) {
            ROCKS_LOG_BUFFER(log_buffer_,
                             "[%s] Gear: compacting for file num -- %u\n",
  ...
  if (c == nullptr) {
    if ((c = PickDeleteTriggeredCompaction()) != nullptr) {
      ROCKS_LOG_BUFFER(log_buffer_,
                       "[%s] Gear: delete triggered compaction\n",
                       cf_name_.c_str());
    }
  }
...
// validate that all the chosen files of L0 are non overlapping in time
  ...
  TEST_SYNC_POINT_CALLBACK("GearCompactionBuilder::PickCompaction:Return",
                           c);
  return c;
}

uint32_t GearCompactionBuilder::GetPathId(
...
Compaction* GearCompactionBuilder::PickCompactionToReduceSortedRuns(
  ...
  for (size_t loop = 0; loop < sorted_runs_.size(); loop++) {
    ...
    if (candidate_count >= (unsigned int)min_merge_width) {
      ...
      for (size_t i = loop;
        ...
        ROCKS_LOG_BUFFER(log_buffer_, "[%s] Gear: Skipping %s",
  ...
  for (size_t i = start_index; i < first_index_after; i++) {
    ...
    ROCKS_LOG_BUFFER(log_buffer_, "[%s] Gear: Picking %s",
                     cf_name_.c_str(), file_num_buf);
  }
...
//
Compaction* GearCompactionBuilder::PickCompactionToReduceSizeAmp() {
  ...
  for (size_t loop = 0; loop + 1 < sorted_runs_.size(); loop++) {
    ...
    sr->Dump(file_num_buf, sizeof(file_num_buf), true);
    ROCKS_LOG_BUFFER(log_buffer_,
                     "[%s] Gear: skipping %s[%d] compacted %s",
                     cf_name_.c_str(), file_num_buf, loop,
  ...
  }
  {
    ...
    ROCKS_LOG_BUFFER(
        log_buffer_,
        "[%s] Gear: First candidate %s[%" ROCKSDB_PRIszt "] %s",
  ...
  for (size_t loop = start_index; loop + 1 < sorted_runs_.size(); loop++) {
    ...
    if (sr->being_compacted) {
      ...
      sr->Dump(file_num_buf, sizeof(file_num_buf), true);
      ROCKS_LOG_BUFFER(
          log_buffer_, "[%s] Gear: Possible candidate %s[%d] %s",
  ...
  if (candidate_size * 100 < ratio * earliest_file_size) {
    ROCKS_LOG_BUFFER(
        log_buffer_,
        "[%s] Gear: size amp not needed. newer-files-total-size %" PRIu64
    ...
    ROCKS_LOG_BUFFER(
        log_buffer_,
        "[%s] Gear: size amp needed. newer-files-total-size %" PRIu64
        " earliest-file-size %" PRIu64,
        cf_name_.c_str(), candidate_size, earliest_file_size);
  }
...
Compaction* GearCompactionBuilder::PickPeriodicCompaction() {
  ROCKS_LOG_BUFFER(log_buffer_, "[%s] Gear: Periodic Compaction",
                   cf_name_.c_str());
  ...
  if (start_index == sorted_runs_.size() - 1) {
    ...
    if (!included_file_marked) {
      ROCKS_LOG_BUFFER(log_buffer_,
                       "[%s] Gear: Cannot form a compaction covering file "
  ...
  TEST_SYNC_POINT_CALLBACK(
      "GearCompactionPicker::PickPeriodicCompaction:Return", c);
